...
import RCProjectListItem from './RCProjectListItem';
import { projectTabs } from './constants';
import './rcproject-list-group.scss';
...
const RCProjectListGroup = ({
  isMapDisplayed,
  selectedStateName,
  noListDots,
}: IProjectListProps) => {
  const stateInfo = useAppSelector((state) => state?.stateSlice);
  const resourceInfo = useAppSelector(
    (state) => state?.resourceConcernSlice?.searchInput
  );
  ...
  let inputdata: ISearchData = {
    ...
    resource_concern_category_id: Number(category),
    resource_concern_id: Number(individual),
  };

  if (window.location.pathname.includes('/search-results')) {
    inputdata = resourceInfo;
  }
  ...
  if (window.location.pathname.includes('/ProjectsAndInitiatives/')) {
    delete inputdata.resource_concern_category_id;
    delete inputdata.resource_concern_id;
  }
...
export default RCProjectListGroup;

RCProjectListGroup.defaultProps = {
  selectedStateName: '',
  noListDots: false,
};
